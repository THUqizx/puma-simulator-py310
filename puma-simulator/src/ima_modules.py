...
class memory (object):
    def __init__ (self, size, addr_offset = 0):
        ...
        # if isinstance(size, float) and str(size).split('.')[1] == '0':
        #     size = int(size)
        # elif not isinstance(size, int):
        #     print("ima_modules495行有问题,size:" + str(size))
        self.size = size
        self.memfile = [''] * size
...
class xb_inMem (object):
    ...
    def write (self, addr, data):
        ...
        # print(f"583_{type(data)}_{data}_{type(cfg.xbdata_width)}_{cfg.xbdata_width}")
